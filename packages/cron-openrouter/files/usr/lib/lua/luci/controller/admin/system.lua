...
function index()
	...
	if nixio.fs.access("/etc/config/fstab") then
		entry({"admin", "Extend", "fstab"}, cbi("admin_system/fstab"), _("Mount Points"), 50)
		entry({"admin", "Extend", "fstab", "mount"}, cbi("admin_system/fstab/mount"), nil).leaf = true
		entry({"admin", "Extend", "fstab", "swap"},  cbi("admin_system/fstab/swap"),  nil).leaf = true
	end
	...
	---live-8
	entry({"admin", "help"}, call("action_help"), _("help"), 81)
	---live-8
...
function action_help()	
	luci.template.render("admin_system/help&firmware")
end
---live-8
